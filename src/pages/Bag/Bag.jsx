...
function Bag({ games, reference }) {
  ...
  const fetchAll = async () => {
    try {
      const shoppingCartsResponse = await fetch(
      ...
      const shoppingCartTicket = shoppingCartsData.map((shoppingCarts) => {
        const ticket = ticketsData.find(
          (ticket) => ticket.ticketId === shoppingCarts.ticketId
        );
      ...
      <>
        <div className="row">
          <div className="table-responsive">
            <table className="shopBagTable table table-borderless ">
              <thead>
                <tr>
                  <th scope="col">No.</th>
              ...
              <tbody>
                {shoppingCarts.map((shoppingCart) => (
                  <tr
                    key={shoppingCart.ticketId === ticket.ticketId}
                    className="shopBagItem"
                  >
        ...
        <div className="row d-flex justify-content-between mt-1">
          <div className="col-lg-2 d-flex align-items-center"></div>
